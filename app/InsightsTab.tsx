...
function formatWeekLabel(dateStr: string) {
  // Parse as local noon to avoid off-by-one day from UTC conversion
  const d = new Date(`${dateStr}T12:00:00`);
...
/**
 * Safely parse a YYYY-MM-DD date string to a friendly display string
 * WITHOUT timezone shifting (the original bug: new Date("2025-06-10") parses
 * as midnight UTC, which in negative-offset timezones shows as June 9).
 */
...
export default function InsightsTab({ customers }: { customers: Customer[] }) {
  ...
  const cardStyle = { ...s.card, padding: isMobile ? 14 : 18, marginBottom: isMobile ? 12 : 16 };
  ...
  const unpaidRevenue = useMemo(
    () => customers.filter((c) => c.completed && !c.paid).reduce((s, c) => s + c.price, 0),
    [customers]
  );
  ...
  /* Best day — FIX: use formatDateDisplay so date string isn't shifted by UTC */
  const bestDay = useMemo(() => {
    ...
    return {
      // formatDateDisplay parses as local midnight, not UTC midnight — no more off-by-one day
  ...
  return (
    <div>
      ...
      {isMobile ? (
        <SwipeRow>
          <div style={{ minWidth: 220, flexShrink: 0 }}><StatBox label="Total Revenue" value={`$${totalRevenue.toLocaleString()}`} /></div>
          <div style={{ minWidth: 220, flexShrink: 0 }}><StatBox label="Jobs Completed" value={completed.length} /></div>
          <div style={{ minWidth: 220, flexShrink: 0 }}><StatBox label="Avg Job Value" value={`$${avgValue}`} /></div>
          <div style={{ minWidth: 220, flexShrink: 0 }}><StatBox label="Unpaid Revenue" value={`$${unpaidRevenue}`} /></div>
          <div style={{ minWidth: 220, flexShrink: 0 }}><StatBox label="Upsell Rate" value={`${upsellRate}%`} /></div>
        </SwipeRow>
      ) : (
      ...
      <div style={cardStyle}>
        <div style={s.cardHeader}>
          ...
          <div style={s.toggle}>
            <button onClick={() => setRevenueView("month")} style={revenueView === "month" ? s.toggleActive : s.toggleBtn}>Monthly</button>
            <button onClick={() => setRevenueView("week")} style={revenueView === "week" ? s.toggleActive : s.toggleBtn}>Weekly</button>
          </div>
        </div>
      ...
      {/* AVG JOB VALUE CHART */}
      <div style={cardStyle}>
        <div style={s.cardHeader}><h3 style={s.cardTitle}>Average Job Value Over Time</h3></div>
      ...
      {/* SERVICES BREAKDOWN */}
      <div style={cardStyle}>
        <div style={s.cardHeader}><h3 style={s.cardTitle}>Most Popular Services</h3></div>
      ...
      {/* PAYMENT METHODS */}
      <div style={cardStyle}>
        <div style={s.cardHeader}><h3 style={s.cardTitle}>Payment Methods</h3></div>
...
const s: any = {
  ...
  statBox: { background: "#fff", borderRadius: 14, padding: "14px 16px", border: "1px solid rgba(0,0,0,0.06)", boxShadow: "0 2px 8px rgba(0,0,0,0.04)" },
  statLabel: { fontSize: 11, opacity: 0.55, marginBottom: 6, textTransform: "uppercase", letterSpacing: 0.5 },
  statValue: { fontSize: 22, fontWeight: 700 },
  highlightCard: { background: "#fff", borderRadius: 14, padding: "14px 16px", border: "1px solid rgba(0,0,0,0.06)", boxShadow: "0 2px 8px rgba(0,0,0,0.04)" },
  ...
  card: { background: "#fff", borderRadius: 16, boxShadow: "0 4px 20px rgba(0,0,0,0.05)", border: "1px solid rgba(0,0,0,0.04)" },
