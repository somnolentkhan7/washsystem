...
export default function Home() {
  ...
  const todayJobs = customers.filter((c) => c.date === todayKey);
  const start = todayJobs[0];

  const routeJobs = !start
    ? []
    : [...todayJobs].sort((a, b) => {
        const distA = Math.hypot((a.lat ?? 0) - (start.lat ?? 0), (a.lng ?? 0) - (start.lng ?? 0));
        const distB = Math.hypot((b.lat ?? 0) - (start.lat ?? 0), (b.lng ?? 0) - (start.lng ?? 0));
        return distA - distB;
      });
  ...
  async function toggleComplete(customer: Customer) {
    const { error } = await supabase.from("customers").update({ completed: !customer.completed }).eq("id", customer.id);
  ...
  /* ---------------- DRAG DROP ---------------- */
  async function moveCustomerToDate(customerId: string, newDate: string) {
    const { error } = await supabase.from("customers").update({ date: newDate }).eq("id", customerId);
    if (error) { console.log(error); return; }
    loadCustomers();
  }
  ...
  /* ---------------- METRICS ---------------- */
  const revenue = customers.filter((c) => c.completed).reduce((sum, c) => sum + c.price, 0);
    ...
    <main style={styles.page}>
      ...
      <div style={styles.tabs}>
        {["dashboard", "jobs", "map", "calendar"].map((t) => (
          <button key={t} onClick={() => setTab(t as any)} style={tab === t ? styles.activeTab : styles.tab}>
            {t.toUpperCase()}
          </button>
        ))}
      </div>
        ...
        <>
          <div style={styles.card}>
            ...
            {todayJobs.length === 0 ? (
              ...
              todayJobs.map((c) => (
                <div key={c.id} style={{ ...styles.item, cursor: "pointer" }} onClick={() => setSelectedCustomer(c)}>
                  <div style={{ ...styles.name, display: "flex", alignItems: "center", gap: 8 }}>
                    {c.name}
                    <span style={{ fontSize: 10, padding: "2px 8px", borderRadius: 999, background: c.completed ? "#dcfce7" : "#fef9c3", color: c.completed ? "#166534" : "#92400e" }}>
            ...
            {routeJobs.length === 0 ? (
              ...
              routeJobs.map((c, i) => (
                <div key={c.id} style={styles.item}>
                  <div style={styles.name}>{i + 1}. {c.name}</div>
                  <div style={styles.sub}>{c.address}</div>
                </div>
              ))
            )}
      ...
      {tab === "jobs" && (
        <div>
          <div style={styles.card}>
            <h3>Add Job</h3>
            <input placeholder="Name" style={styles.input} value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} />
            ...
            <input placeholder="Address" style={styles.input} value={form.address} onChange={(e) => setForm({ ...form, address: e.target.value })} />
            ...
            <div style={{ marginTop: 10 }}>
              ...
              {["Driveway", "Sidewalk", "Patio", "Trashcans"].map((service) => (
                <button key={service} onClick={() => setForm((prev) => ({ ...prev, services: prev.services.includes(service) ? prev.services.filter((s) => s !== service) : [...prev.services, service] }))}
                  style={{ marginRight: 8, marginBottom: 8, padding: "6px 10px", borderRadius: 999, border: "1px solid #ddd", background: form.services.includes(service) ? "#1d1d1f" : "#fff", color: form.services.includes(service) ? "#fff" : "#000", fontSize: 12 }}>
                  {service}
                </button>
              ))}
            </div>
          ...
          <div style={styles.filters}>
            {["all", "pending", "done"].map((f) => (
              <button key={f} onClick={() => setJobFilter(f as any)} style={jobFilter === f ? styles.activeFilter : styles.filter}>
                {f.toUpperCase()}
              </button>
            ))}
          </div>
        ...
        <div>
          ...
          <div style={styles.card}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
              <div>
                <h3 style={{ margin: 0, fontSize: 18 }}>{calendarView === "week" ? "Weekly" : "Monthly"} Calendar</h3>
                <p style={{ opacity: 0.55, marginTop: 4, fontSize: 13 }}>Drag jobs between days to schedule your route</p>
              </div>
              <div style={{ fontSize: 12, padding: "4px 10px", borderRadius: 999, background: "#f3f4f6", color: "#111" }}>
                {calendarView === "week"
                  ...
                  : new Date(new Date().getFullYear(), new Date().getMonth() + weekOffset).toLocaleDateString("en-US", { month: "long", year: "numeric" })
                }
              </div>
            </div>
          ...
          {calendarView === "week" ? (
              ...
              {weekDays.map((day, i) => {
                ...
                return (
                  <div
                    ...
                    onDrop={(e) => { const customerId = e.dataTransfer.getData("customerId"); moveCustomerToDate(customerId, key); setHoverDate(null); }}
                  >
            ...
            (() => {
              ...
              const month = today.getMonth() + weekOffset;
                ...
                <div>
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: 4 }}>
                    {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((d) => (
                      <div key={d} style={{ fontSize: 11, fontWeight: 700, textAlign: "center", opacity: 0.5, paddingBottom: 4 }}>{d}</div>
                    ))}
                    {cells.map((day, i) => {
                      ...
                      return (
                        <div
                          ...
                          onDrop={(e) => { const customerId = e.dataTransfer.getData("customerId"); moveCustomerToDate(customerId, key); setHoverDate(null); }}
                        >
                          <div style={{ fontWeight: 700, marginBottom: 4, opacity: 0.7 }}>{day.getDate()}</div>
                          {jobs.map((c) => (
                            <div key={c.id} draggable onDragStart={(e) => e.dataTransfer.setData("customerId", c.id)}
                              style={{ background: c.completed ? "#dcfce7" : "#fef9c3", borderRadius: 4, padding: "2px 5px", marginBottom: 3, cursor: "grab", fontSize: 10, fontWeight: 600, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                              {c.name}
                            </div>
                          ))}
                        </div>
      ...
      {selectedCustomer && (
        <div onClick={() => setSelectedCustomer(null)}
          style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "rgba(0,0,0,0.4)", display: "flex", alignItems: "center", justifyContent: "center" }}>
          <div onClick={(e) => e.stopPropagation()}
            ...
            <span style={{ display: "inline-block", marginTop: 6, padding: "4px 10px", borderRadius: 999, fontSize: 12, background: selectedCustomer.completed ? "#dcfce7" : "#fef9c3", color: selectedCustomer.completed ? "#166534" : "#92400e" }}>
...
const styles: any = {
  ...
  tabs: { display: "flex", gap: 10, marginBottom: 18, flexWrap: "nowrap", overflowX: "auto", WebkitOverflowScrolling: "touch" },
  tab: { padding: "8px 14px", borderRadius: 999, border: "1px solid rgba(0,0,0,0.08)", background: "#fff", fontSize: 13, cursor: "pointer" },
  activeTab: { padding: "8px 14px", borderRadius: 999, background: "#1d1d1f", color: "#fff", fontSize: 13, cursor: "pointer" },
  ...
  draggingItem: { opacity: 0.5, transform: "scale(0.98)" },
...
const calBtn = {
  padding: "6px 12px", borderRadius: 10, border: "1px solid #e5e7eb", background: "#fff", fontSize: 12, cursor: "pointer",
};

const calBtnPrimary = {
  padding: "6px 12px", borderRadius: 10, border: "1px solid #111", background: "#111", color: "#fff", fontSize: 12, cursor: "pointer",
};
